...
echo " "
echo "This script will remove versions of Node.js prior to version 7.x, and Node-RED"
echo "and if necessary replace them with Node.js 10.x LTS (9.x for x86) and the latest"
echo "Node-RED from Npm."
echo " "
...
case $yn in
    [Yy]* )
        ...
        if [[ -e $NODERED_HOME/.nvm ]]; then
            ...
            echo -ne "  Update Node.js LTS                  $CHAR"
        elif [ "$nv" = "v0" ] || [ "$nv" = "v1" ] || [ "$nv" = "v3" ] || [ "$nv" = "v4" ] || [ "$nv" = "v5" ] || [ "$nv" = "v6" ] || [ "$nv" = "v7" ] || [ "$nv" = "v9" ] || [ "$nv2" = "v8.11.1" ]; then
            if [ -e /etc/apt/sources.list.d/nodesource.list ]; then
                if [ "$nv" = "v0" ] || [ "$nv" = "v1" ] || [ "$nv" = "v3" ] || [ "$nv" = "v4" ] || [ "$nv" = "v5" ] || [ "$nv" = "v6" ] || [ "$nv" = "v7" ] ||[ "$nv" = "v9" ] || [ "$nv2" = "v8.11.1" ]; then
                    ...
                    if curl -sL https://deb.nodesource.com/setup_9.x | sudo -E bash - 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null; then CHAR=$TICK; else CHAR=$CROSS; fi
                else
                    CHAR="-"
                fi
                ...
                if cat /proc/cpuinfo | grep model | grep -q ARMv6 ; then
                    ...
                    curl -sL https://deb.nodesource.com/setup_9.x | sudo -E bash - 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null
                    if sudo apt-get install -y nodejs 2>&1 | sudo tee -a /var/log/nodered-install.log >>/dev/null; then CHAR=$TICK; else CHAR=$CROSS; fi
